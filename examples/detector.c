...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
	...
	while(1){
		...
		for (int i = 0; i < l.w*l.h*l.n; i++)
		{
			...
			FILE *fptrCar = NULL;
			FILE *fptrTL = NULL;
			static unsigned char isExistTL = 0;
			static unsigned char isExistCar = 0;
			int classes = max_index(probs[i], l.classes);
			if (!strcmp("car", names[classes]) ||
					!strcmp("motorbike", names[classes]) || 
					!strcmp("bus", names[classes]) ||
					!strcmp("truck", names[classes]))
			{
				if (!isExistCar)
				{
					fptrCar = fopen("cars_boxlist.txt", "w");
					if (fptrCar)
						fprintf(fptrCar, "{left:%d,right:%d,top:%d,bottom:%d}\n", left, right, top, bot);
					fclose(fptrCar);
					isExistCar = 1;
				}
				else
				{
					fptrCar = fopen("cars_boxlist.txt", "a+");
					if (fptrCar)
						fprintf(fptrCar, "{left:%d,right:%d,top:%d,bottom:%d}\n", left, right, top, bot);
			...
			else if(!strcmp("traffic light", names[classes]))
			{
				if (!isExistTL)
				{
					fptrTL = fopen("tl_boxlist.txt", "w");
					if (fptrTL)
						fprintf(fptrTL, "{left:%d,right:%d,top:%d,bottom:%d}\n", left, right, top, bot);
					fclose(fptrTL);
					isExistTL = 1;
				}
				else
				{
					fptrTL = fopen("tl_boxlist.txt", "a+");
					if (fptrTL)
						fprintf(fptrTL, "{left:%d,right:%d,top:%d,bottom:%d}\n", left, right, top, bot);
